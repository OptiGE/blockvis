(async () => {
            ...
            annotations: [
              {
                type: 'line',
                xMin: 0,
                xMax: Math.max(...data.map(({ mileage }) => mileage)),
                yMin: Math.max(...data.map(({ price }) => price)),
                yMax: 0,
              ...
              },
              {
                type: 'line',
                xMin: 0,
                xMax: Math.max(...data.map(({ mileage }) => mileage)) * 4 / 5,
                yMin: Math.max(...data.map(({ price }) => price)) * 4 / 5,
                yMax: 0,
                borderColor: 'rgba(0, 128, 128, 0.6)',
                borderWidth: 1,
              },
    ...
    // EXTRA CHART WITH PRICE/MILE PLOTTED FROM HERE ON DOWN

    // Create a second canvas element for the new chart
    const canvas2 = document.createElement('canvas');
    canvas2.width = 800;
    canvas2.height = 600;
    document.body.appendChild(canvas2);
  
    // Calculate price/mileage quotients
    const priceMileageQuotients = data.map(({ price, mileage }) => mileage / price);
  
    // Create a scatter plot with the price/mileage quotients and years
    ...
    const chart2 = new Chart(ctx2, {
      ...
      data: {
        datasets: [
          {
            data: data.map(({ price }, index) => ({ x: priceMileageQuotients[index], y: price })),
            borderColor: data.map(({ color }) => color),
            backgroundColor: data.map(({ color }) => color),
            pointRadius: 5,
            pointHoverRadius: 7,
            ...
            callbacks: {
              ...
              label: (context) => {
                ...
                return [
                  ...
                  `Driven Kilometers: ${mileage}`,
                  `Year: ${year}`,
                  `Miles/Crown: ${priceMileageQuotients[context.dataIndex].toFixed(2)}`
                ];
              }
            }
          },
