...
async function runVisualization() {
    ...
    await Promise.all([
        ...
        new Promise((resolve, reject) => {
            ...
            script2.src = 'https://cdn.jsdelivr.net/npm/chartjs-plugin-annotation@1.1.0';
...
function waitForListings(callback) {
    ...
    const checkListings = () => {
      ...
      console.log('Checking listings:', listings.length);
      if (listings.length > 0 || tries >= MAX_TRIES) {
        callback();
      } else {
        tries++;
  ...
  async function main() {
    if (window.location.hostname === 'www.blocket.se' || window.location.hostname === 'blocket.se') {
        console.log("3");
        await waitForListings(runVisualization);
    }
}
